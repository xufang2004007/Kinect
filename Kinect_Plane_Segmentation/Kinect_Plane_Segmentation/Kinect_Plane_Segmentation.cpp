...
{
	int ADS_Connection_Done = ADS_Connection();
...
int ADS_Connection()
{
	...
	/*if (!nErr)
	{
		cout << "ADS Connection Completion!" << "	" << "Address is : " << pAddr << endl;
		return 1;
	}*/
...
void Plane_segmentation()
{
	...
	nErr = AdsSyncWriteReq(pAddr, 0x4020, 0x20, 0x4, &a);
	b = coefficients->values[1];
	nErr = AdsSyncWriteReq(pAddr, 0x4020, 0x24, 0x4, &b);
	c = coefficients->values[2];
	nErr = AdsSyncWriteReq(pAddr, 0x4020, 0x28, 0x4, &c);
	d = coefficients->values[3];
	nErr = AdsSyncWriteReq(pAddr, 0x4020, 0x2C, 0x4, &d);
